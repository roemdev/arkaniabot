...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const reason = interaction.options.getString('reason') || 'No se ha especificado una razón.';
    ...
    if (subcommand === 'ban') {
      ...
      try {
        ...
        await interaction.reply({ content: 'Ocurrió un error al intentar desbanear al usuario.', ephemeral: true });
      }
      ...
      const warnEmbed = new EmbedBuilder()
        .setColor('#FFC868')
        .setTitle(' ')
        .setDescription(`${memberTarget} ha sido advertido. Motivo: **${reason}**`);

      const logEmbed = new EmbedBuilder()
        .setColor('#FFC868')
        .setTitle('Usuario Advertido')
        .addFields(
          { name: 'Usuario', value: `${memberTarget.user.tag} (${memberTarget.id})`, inline: true },
      ...
      try {
        ...
        await interaction.reply({ content: `**${memberTarget.user.tag}** ha sido advertido.`, ephemeral: true });
      } catch (error) {
